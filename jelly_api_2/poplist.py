...
poplist = Blueprint('api2_poplist', __name__)

@poplist.route("/poplist", methods=['GET'])
@poplist.route("/poplist/", methods=['GET'])
def api2_poplist(pop_name=None, with_srvtype=None, with_hostname=None, with_status=None, exact=False):

    meta_info = dict()
    meta_info["version"] = 2
    meta_info["name"] = "Pop List for Jellyfish2 API Version 2"
    ...
    if "pop_name" in request.args :
        try:
            pop_name_regex = ast.literal_eval(request.args["pop_name"])
        except Exception as e :
            argument_error = True
            error_dict["pop_name_parse_error"] = "Can not parse pop_name"
        else:
            where_args_params.append(pop_name_regex)
            if useRegex :
    ...
    if "with_srvtype" in request.args :
        try:
            srvtype_regex = ast.literal_eval(request.args["with_srvtype"])
        except Exception as e :
            argument_error = True
            error_dict["with_srvtype_parse_error"] = "Can not parse with_srvtype"
        else:
            where_args_params.append(srvtype_regex)
            if useRegex :
    ...
    requesttype = "pop_list"

    links_info = dict()

    links_info["self"] = g.config_items["v2api"]["preroot"] + g.config_items["v2api"]["root"] + "/poplist"
    ...
    if argument_error == False :
        meta_info["this_cached_file"] = g.config_items["v2api"]["cachelocation"] + "/poplist_" + cache_string + ".json"
    ...
    pop_list_query='''select DISTINCT pop
                        from hosts '''

    pop_list_query = pop_list_query + where_full_string


    # Select Query
    if do_query and argument_error == False :
        #print(g.cur.mogrify(pop_list_query, where_args_params))
        g.cur.execute(pop_list_query, where_args_params)
        all_pops = g.cur.fetchall()
        amount_of_pops = len(all_pops)
    else :
    ...
    if amount_of_pops > 0 :
        ...
        for i in range(0, len(all_pops)) :
            this_results = dict()
            this_results["type"] = requesttype
            this_results["id"] = all_pops[i]["pop"]
            this_results["attributes"] = all_pops[i]
            this_results["relationships"] = dict()
            this_results["relationships"]["hostsof"] = "{}{}/hostsearch/?pop='{}'&exact=True".format(g.config_items["v2api"]["preroot"],\
