...
def api2_custdashboard_create(dash_id=None, modifyorder=None):
    ...
    try:
        ...
        try:
            ...
            custdash_list_content_string = custdash_list_content.decode("utf-8")
            audit_list_content_object = json.loads(audit_list_content_string)
            custdash_list_content_object = json.loads(custdash_list_content_string)
        except Exception as e :
            api_error = True
            error_dict["api_read_error"] = "Trouble reading data from endpoints. " + str(e)
        else :
            # Let's generate lists validation lists
            valid_audit_ids = [ id["attributes"]["audit_id"] for id in audit_list_content_object["data"] ]
            valid_custdash_ids = [ id["attributes"]["custdashboardid"] for id in custdash_list_content_object["data"] ]
                ...
                if "add" in modifyorder.keys() or "remove" in modifyorder.keys() :
                    ...
                    if "add" in modifyorder.keys() :
                        ...
                        if type(modifyorder["add"]) is list :
                            possible_id_list = [ id for id in modifyorder["add"] if type(id) is int and id > 0 and id in valid_audit_ids ]
                    ...
                    if "remove" in modifyorder.keys() :
                        if type(modifyorder["remove"]) is list:
                            possible_id_list = [ id for id in modifyorder["remove"] if type(id) is int and id > 0 and id in valid_audit_ids ]
                            if len(possible_id_list) > 0 :
                                # There are IDs
                                do_remove = True
