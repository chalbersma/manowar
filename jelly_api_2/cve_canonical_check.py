...
def api2_cve_canonical_check(cve_name=None, hostname=False, pop=False, srvtype=False, \
    ...
    links_info["self"] = g.config_items["v2api"]["preroot"] + g.config_items["v2api"]["root"] + "/cve/canonical_check/"
    ...
    if argument_error == False :
        meta_info["this_cached_file"] = g.config_items["v2api"]["cachelocation"] + "/cve_check_" + cache_string + ".json"
        ...
        if this_cve_data.get("priority", "Untriaged") in ["Untriaged"] :
            ...
            for pkg_name in this_cve_data["packages"].keys() :
                ...
                for release in this_pkg_obj["releases"].keys() :
                    ...
                    if this_pkg_release_obj["status"] in ["needed", "needs-triage"] :
                        ...
                        rel_dict[release]["packages_pending_release"].append(pkg_name)
            ...
            for release in rel_dict.keys() :
                ...
                if rel_dict[release]["packages_to_check"] == False :
                    ...
                    # Something to Check!
                    generic_large_compare_query = g.config_items["v2api"]["root"] + "/genericlargecompare/?"
                    ...
                    glc_private = "{}{}{}".format(g.HTTPENDPOINT, generic_large_compare_query, query_string)

                    content = requests.get(glc_private)

                    content_json = content.json()

                    meta_info["glc_Endpoint"].append("{}{}{}".format(str(g.config_items["v2api"]["preroot"]), generic_large_compare_query , query_string))

                    if "errors" in content_json.keys():
                        meta_info["release_error"] = "Errors getting releases for {}.".format(release)
                        found_hosts = 0
                        results_array = list()
                    else :
                        results_array = [ hostdict for hostdict in content_json["data"] ]
