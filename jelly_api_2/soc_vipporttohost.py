...
def api2_soc_vipporttohost(ctype="none"):
    ...
    if "port" in request.args :
        try:
            ...
            error_dict["port_read_error"] = "Error parsing port filter" + str(e)
            argument_error=True
        else :
            if type(port) != int :
                argument_error=True
    ...
    if "ipv" in request.args :
        try:
            ...
            error_dict["ipv46_read_error"] = "Error parsing ipv directive" + str(e)
            argument_error=True
        else :
            if type(ipv) != int :
                argument_error=True
    ...
    if "protocol" in request.args :
        try:
            ...
            error_dict["protocol_read_error"] = "Error parsing protocol filter" + str(e)
            argument_error=True
        else :
            if protocol not in ["udp", "tcp"] :
                argument_error=True
                error_dict["protocol_error"] = "Protocol must be udp or tcp not : " + str(protocol)
    else :
        # Default is TCP
        protocol="tcp"
    ...
    if argument_error == False :
        ...
        try:
            ...
            meta_dict["this_cached_file"] = g.config_items["v2api"]["cachelocation"] + "/soc_vipporttohost_" + cache_string + ".json"

    links_dict["parent"] = g.config_items["v2api"]["preroot"] + g.config_items["v2api"]["root"] + "/soc"
    links_dict["self"] = g.config_items["v2api"]["preroot"] + g.config_items["v2api"]["root"] + "/soc/vipporttohost/"
