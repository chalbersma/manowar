...
## Jellyfish 2 Overview

Jellyfish is a follows the Unix philosophy of designing small modules to do a job
...
`jellyfish_storage.json.schema` (also stored in SVN). You can utilize the
...
`jellyfish_storage.json.schema` specification) and "do the right thing" for storage.
...
The Database the storage module uses is a MariaDB 10.1 database. It's connection
details are configured in `storage.ini`. Additionally it's schema is stored in the
`/EdgeCast/secops/setup/jellyfish2_db_schema.sql`.
...
a jinja template (currenltly located at `jellyfish2/swagger/api.yaml.jinja` in SVN)
to build a Swagger definition file from it. Currently it's used for the
`/EdgeCast/secops/jelly_api_2` directory to pull a file that get's displayed here.

### Storage JSON Verify

Is a module that will check a particular JSON file against json schema file
(`jellyfish_storage.json.schema` in SVN) to see if it's valid. In theory it can
