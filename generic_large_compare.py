...
def generic_large_compare(db_conn, host_list_dict, mtype, ctype, csubtype, \
    ...
    if len(massaged_ctype) != len(massaged_csubtype) \
        ...
        for index_value in range(0, len(massaged_ctype)):
            ...
            comparison = []
            comparison.append("select fk_host_id, collection_value from collection")
            comparison.append("where collection_type = '" + str(massaged_ctype[index_value]) + "' ")
            ...
            comparison.append(" group by fk_host_id order by last_update desc ;")
