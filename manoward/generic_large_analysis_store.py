...
def generic_large_analysis_store(db_conn, audit_id, audit_results_dict, FRESH):
    ...
    for bucket in audit_results_dict.keys() :
        ...
        for host in audit_results_dict[bucket] :
            if "pfe" in host.keys() :
                audit_results_updt[bucket].append( [ host["host_id"], host["pfe"], host["pfevalue"] ] )
    ...
    for bucket in audit_results_keys.keys() :
        ...
        bucket_ids_paramertization_list = [ " %s " for x in audit_results_keys[bucket] ]

        bucket_ids_paramertization_list_string = " , ".join(map(str, bucket_ids_paramertization_list))
        ...
        pull_current = list()
        pull_current.append("SELECT audit_result_id, fk_host_id, audit_result, audit_result_text")
        pull_current.append("from audits_by_host")
        pull_current.append("where bucket = %s  ")
        pull_current.append("and fk_host_id in (" + bucket_ids_paramertization_list_string + ")")
        ...
        this_bucket_update = [ audit_result for audit_result in current_audits_list if audit_result[2] == audit_results_updt[bucket][audit_results_keys[bucket].index(audit_result[1])][1] and audit_result[3] == audit_results_updt[bucket][audit_results_keys[bucket].index(audit_result[1])][2] ]

        # Create an index to reference this_bucket_update
        this_bucket_update_index = [ update_host_id[1] for update_host_id in this_bucket_update ]
        ...
        this_bucket_inserts = [ ( -1, new_audit_result[0], new_audit_result[1], new_audit_result[2] ) for new_audit_result in audit_results_updt[bucket] if new_audit_result[0] not in this_bucket_update_index ]
        ...
        try:
            ...
            if len(this_bucket_update_ids) > 0 :
                ...
                update_query_parameter_string_list=[ " %s " for x in this_bucket_update_ids ]
                update_query_parameter_string=" , ".join(map(str, update_query_parameter_string_list))
        ...
        try:
            if len(this_bucket_inserts) > 0 :
                query_data = [ [ result[1], audit_id, ANALYZE_TIME, ANALYZE_TIME,  bucket, result[2], result[3] ] for result in this_bucket_inserts ]
                #print("Insert Data : " , query_data)
                # Only do this is there's stuff.
                insert_query = []
                insert_query.append("INSERT into audits_by_host ( fk_host_id, fk_audits_id, initial_audit, last_audit, bucket, audit_result, audit_result_text ) " )
