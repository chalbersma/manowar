...
def insert_update_audit(db_config_items, audit):
    ...
    """
    Do the Needful to Insert/Update the Audit Specified
    """
    ...
    columns = """audit_name, audit_short_description, audit_long_description,
                ...
                audit_comparison, audit_priority, e """
...
def audit_load(CONFIGDIR, CONFIG):

    """
    Audit Load. Look through the Disk and Load the Audits that Matter.
    """
    
    logger = logging.getLogger("analyz.py:audit_load")
        ...
        for auditfile in auditfiles:
            ...
            for found_audit_name in these_audits.keys():
                if found_audit_name in audits.keys():
                    ...
                    audits[found_audit_name] = audittools.AuditSource(source_key=found_audit_name,
                                                                      audit_name=found_audit_name,
                                                                      **these_audits[found_audit_name])

    for item_key, this_audit in audits.items():
        logger.debug(this_audit)

        logger.debug("Valid {}".format(this_audit.validate_audit_live()))

        results[item_key] = this_audit.update_db(db_cur)

        logger.debug("Update Result : {}".format(results[item_key]))

    return results

def dequeue_audits(db_config_items, list_of_hosts, audit_queue):

    """
    Dequeue Audits

    Spins out a Thread that
    """

    logger = logging.getLogger("analyze:dequeue_audits")
    ...
    while True:
        ...
        try:
            ...
            logger.error("Failure to Create Manager for audit {}".format(audit_obj))
            logger.debug("{}".format(multiprocess_error))
            audit_queue.task_done()

            return
        else:
            return_dict = manager.dict(host_inserts=0, host_updates=0)

        try:
            try:
                analyze_audit_process = multiprocessing.Process(target=analyze_one_audit, args=(db_config_items, list_of_hosts, audit_obj, return_dict))
                analyze_audit_process.name = audit_obj.audit_name
                analyze_audit_process.daemon = True
                analyze_audit_process.start()
            except Exception as analyze_pid_error:
                logger.error("Error with Analyze Audit {} : {}".format(audit_obj, analyze_pid_error))

                analyze_audit_process.terminate()
            else:

                while multiprocessing.Process.is_alive(analyze_audit_process) == True:
                    logger.debug("Waiting for: {} {}".format(audit_obj, multiprocessing.Process.is_alive(analyze_audit_process)))
            ...
            logger.error("Failure to Analyze Audit {} : {}".format(audit_obj,
                                                                   audit_analyisis_error))

        logger.debug(return_dict)
        try:
            with audit_host_counts_lock:
                logger.info("{} I:{} U:{}".format(audit_obj,
            ...
            # print(return_dict)
            logger.error("Failure on Audit when Recording Metrics {} : {}".format(audit_obj,
                                                                                  metrics_error))
        audit_queue.task_done()

    return

def analyze_all_audits(db_config_items, audit_queue, all_audits, list_of_hosts, FRESH, MAXTHREADS):

    """
    Analyze All Audits.
    Cycle thourgh the audits and Spin Up Threads to Evaluate them.

    It spins up a seperate multiprocess thread to get around the GIL locking on process and such
    """
    ...
    for x in range(MAXTHREADS):
        # This is the magic. It calls dequeu hosts with the args noted
        t = threading.Thread(target=dequeue_audits, args=(db_config_items, list_of_hosts, audit_queue))
    ...
    for audit_name, audit_obj in all_audits.items():
        # Populate Audit Queue
        logger.info("About to Queue audit : {}".format(audit_name))
        # try:
        audit_queue.put(audit_obj)
...
def analyze_one_audit(db_config_items, list_of_hosts, audit_obj, return_dict):

    """
    This is the target of a multiprocess process that will analyze just one audit against
    the population given in list_of_hosts
    """

    logger = logging.getLogger("analyze_one_audit")

    logger.debug("Attempting to Analyze Audit {}".format(audit_obj))

    fresh = db_config_items["analyze"]["freshseconds"]
    ...
    try:
        ...
        logger.debug("{} Preparing to Analyze {} Hosts with {} Buckets".format(audit_obj,
                                                                               len(items_left_to_bucket),
                                                                               len(audit_obj.buckets())))

        for bucket in audit_obj.buckets():
            host_buckets[bucket] = list()
            host_comparison[bucket] = list()

            logger.debug("{} Processing Bucket {}".format(audit_obj, bucket))

            this_mtype = audit_obj.audit_data["filters"][bucket]["filter-match"]
            this_ctype = audit_obj.audit_data["filters"][bucket]["filter-collection-type"]
            this_csubtype = audit_obj.audit_data["filters"][bucket]["filter-collection-subtype"]
            this_mvalue = audit_obj.audit_data["filters"][bucket]["filter-match-value"]

            try:
                bucket_results = generic_large_compare(db_conn,
                                                       ...
                                                       fresh,
                                                       exemptfail=True,
                                                       audit_name=audit_obj.audit_name)
            except Exception as glc_bucket_results_error:
                logger.error("Error on Generic Large Compare on bucket {} : audit {}".format(bucket, audit_obj))
                logger.warning("Maybe no Hosts for Bucket {} on audit {}".format(bucket, audit_obj))
                ...
                # Process All the Passing Hosts and pull them out of the list
        ...
        # I now Have all my hosts into their appropriate Buckets. It's time to
        # Do the Comparisons to find Actual passings and Failings
        for comparison in host_buckets.keys():

            if len(host_buckets[comparison]) > 0:
                try:
                    this_mtype = audit_obj.audit_data["comparisons"][comparison]["comparison-match"]
                    this_ctype = audit_obj.audit_data["comparisons"][comparison]["comparison-collection-type"]
                    this_csubtype = audit_obj.audit_data["comparisons"][comparison]["comparison-collection-subtype"]
                    this_mvalue = audit_obj.audit_data["comparisons"][comparison]["comparison-match-value"]

                except Exception as comparison_error:
                    logger.error("Error grabbing comparisons for audit {} : {}".format(audit_obj, comparison_error))
                else:
                    # Check What Type
                    logger.debug("{} Running Comparison on Bucket {}/{}".format(audit_obj, comparison, len(host_buckets[comparison])))

                    if this_mtype in ["subnonhere", "suballhere", "subknowall"]:
                        # Add Massive Subtype
                        try:
                            comparison_results = subtype_large_compare(db_conn,
                                                                       host_buckets[comparison],
                                                                       this_mtype,
                                                                       this_ctype,
                                                                       this_csubtype,
                                                                       this_mvalue,
                            ...
                            logger.error("{} Error on Subtype Large Compare on Comparison for bucket {}".format(audit_obj,
                                                                                                                comparison))
                        ...
                        # Generic Comparison Use the GLC Module
                        try:
                            comparison_results = generic_large_compare(db_conn,
                                                                       host_buckets[comparison],
                                                                       this_mtype,
                                                                       this_ctype,
                                                                       this_csubtype,
                            ...
                            logger.error("{} Error on Generic Large Compare on Comparison for bucket {}".format(audit_obj,
                                                                                                                comparison))
                ...
                logger.info("No found hosts for bucket {} in audit {}".format(comparison, audit_obj))

        #bucket in host_bucket
        #print(auditName, " Results : ", host_comparison)
        logger.debug(host_comparison)
        logger.info(host_comparison)
        massinserts, massupdates = generic_large_analysis_store(db_conn,
                                                                audit_obj.audit_id,
                                                                host_comparison,
                                                                fresh)
        ...
        logger.error("Error doing analyze for {} : {}".format(audit_obj, analyze_error))
        logger.debug(analyze_error)
        # Exit My Thread with an Error
        sys.exit(1)
    else:
        # Exit My Thread Successfully
        sys.exit(0)

def analyze(CONFIG, newer=-1, shard=None, **kwargs):
    ...
    args_clause = list()
    args_clause_args = list()


    if newer > 0:
        midnight = (int(time() // 86400)) * 86400
        beat_ts =  midnight - (86400 * newer)
        args_clause.append(" audit_ts > FROM_UNIXTIME(%s) ")
        args_clause_args.append(beat_ts)
    if shard is not None:
        args_clause.append(" audit_uuid REGEXP %s")
        args_clause_args.append("^{}".format(shard))
    if isinstance(kwargs.get("name_regex", None), str):
        args_clause.append(" audit_name REGEXP %s ")
        args_clause_args.append(kwargs["name_regex"])


    if len(args_clause) > 0:
        where_string = " where "
    else:
        where_string = ""

    get_audit_query = '''select audit_id, audit_name, audit_uuid
                         from audits
                         {} {}'''.format(where_string,
                                         " and ".join(args_clause))
    ...
    if run_result["has_error"] is True:
        logger.error("Unable to Query Database for Audits")
        logger.debug("Newer Setting: {}".format(newer))
        logger.debug("Shard Setting: {}".format(shard))
    else:
        for ta in run_result.get("data", list()):

            this_audit_obj = audittools.AuditSource(do_db=True,
                                                    db_cur=db_cur,
                                                    **ta)

            if this_audit_obj.validate_audit_live() is True:
                logger.info("Adding Audit {} to my todo List".format(this_audit_obj.audit_name))
                logger.debug("Audit UUID {} ".format(this_audit_obj.audit_uuid))

                audits[this_audit_obj.audit_name] = this_audit_obj
            else:
                logger.warning("Audit {} not being evaluated as it has failed the validation pre flight check.".format(this_audit_obj.audit_name))
    ...
    if host_good:
        ...
        analyze_stats["jobtime"] = analyze_all_audits(config_items, audit_queue, audits, host_list, FRESH, MAXTHREADS)
        analyze_stats["audit_inserts"] = audit_host_inserts
        analyze_stats["audit_updates"] = audit_host_updates
