...
display_custdashboard_modify = Blueprint('display2_custdashboard_modify', __name__)
...
def display2_custdashboard_modify(dash_id=False):
    ...
    if argument_error == False :
        try:
            ...
            this_custdashboard_endpoint = "/v2/custdashboard/dashboard/{}/?".format(str(dash_id))
            this_custdashboard_general_endpoint = "/v2/custdashboard/list/{}/?".format(str(dash_id))
            ...
            this_private_custdashboard_general_endpoint = g.HTTPENDPOINT + this_custdashboard_general_endpoint
        except Exception as e:
            error_dict["query_string_error"] = "Error generating self api: " + str(e)
            argument_error = True
        else:

            try:
                audits_list_text = requests.get(this_private_auditlist_endpoint).text
                custdashboard_list_text = requests.get(this_private_custdashboard_endpoint).text
                custdashboard_general_text = requests.get(this_private_custdashboard_general_endpoint).text
                ...
                if "data" in custdashboard_list.keys() :
                    current_audits_id_validation = [ caudit["attributes"]["fk_audits_id"] for caudit in custdashboard_list["data"] ]
                ...
                avail_audits = [ audit for audit in audits_list["data"] if audit["id"] not in current_audits_id_validation ]
