...
display_cve_canonical_check_results = Blueprint('display2_cve_canonical_check_results', __name__)
...
def display2_cve_canonical_check_results(exact=False, hostname=False, hoststatus=False, pop=False, srvtype=False, cve_name=False):
    ...
    if "cve_name" in request.args :
        try:
            ...
            error_dict["cve_name_read_error"] = "Error parsing hostname " + str(e)
            argument_error=True
    ...
    if argument_error == False :
        try:
            cve_check_local_query = g.config_items["v2api"]["root"] + "/cve/canonical_check/?"

            query_string = urllib.parse.urlencode(query_args)
            meta_dict["args"] = query_args

            cve_check_private = "{}{}{}".format(g.HTTPENDPOINT, cve_check_local_query, query_string)
