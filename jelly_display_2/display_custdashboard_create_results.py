...
display_custdashboard_create_results = Blueprint('display2_custdashboard_create_results', __name__)
...
def display2_custdashboard_create_results(dashboard_name=None, dashboard_description=None):
    ...
    if argument_error == False :
        try:
            ...
            query_string_bits["dashboard_description"] = "'''{}'''".format(dashboard_description)

            print(query_string_bits)

            query_string=urllib.parse.urlencode(query_string_bits)
            meta_dict["query_string"] = query_string
            this_endpoint = g.config_items["v2api"]["root"] + "/custdashboard/create/?" + query_string
