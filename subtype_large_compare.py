...
def subtype_large_compare(db_conn, host_list_dict, mtype, ctype, csubtype, mvalue, FRESH):
	...
	if type(csubtype) is str :
		...
		massaged_csubtype = [ ast.literal_eval('"' + item.replace("," , "\",\"") + '"') for item in csubtype ]
	...
	if type(mvalue) is str :
		...
		massaged_mvalue = [ ast.literal_eval('"' + item.replace("," , "\",\"") + '"') for item in mvalue ]
	...
	for index_value in range(0, len(massaged_ctype)) :
		...
		collection = []
		collection.append("SELECT fk_host_id, count(DISTINCT(collection_subtype))")
		...
		collection.append("	( collection_type = '" + str(massaged_ctype[index_value]) + "' )")
		collection.append(" AND ")
		collection.append(" ( ")
		...
		for sub_index_value in range(0, len(massaged_csubtype[index_value])) :
			...
			if massaged_mvalue[index_value][sub_index_value] == "any" :
				...
				matchstring = " AND collection_value REGEXP '" + massaged_mvalue[index_value][sub_index_value] + "'"

			#print("Column Match: ", matchstring)

			columnmatch_string = "collection_subtype " + COLUMNMATCH +  "'" + massaged_csubtype[index_value][sub_index_value] + "'"
		...
		collection.append(" and last_update >= now() - INTERVAL " + str(FRESH) + " SECOND " )
		...
		try:
			if mtype == "subnonhere" :
				...
				exempthost = []
				passhost = [ host for host in host_ids_list if host not in query_results_list_index ]
				failhost = [ host[0] for host in query_results_list if host[1] > 0 ]
			elif mtype == "suballhere" :
				# All the hosts that Aren't in our query Results Fail
				exempthost = [ ]
				# All the hosts in the results whose output matches exactly
				passhost = [ host[0] for host in query_results_list if host[1] == len(massaged_csubtype[index_value]) ]
				# All the hosts in our query results not in exempthosts or passhosts
				failhost = [ host[0] for host in query_results_list if host[0] not in exempthost and host[0] not in passhost ]
			elif mtype == "subknowall" :
				...
				raise Exception("Unknown match type. Potential Race Condition!")
		...
		for host in range(0,len(results_dict)):
			...
			try:
				if "pfe" in results_dict[host].keys() :
					...
					results_dict[host]['pfevalue'] = "Subtype Comparison " + mtype + " Failed"
				elif results_dict[host]['host_id'] in passhost :
					results_dict[host]['pfe'] = "pass"
					results_dict[host]['pfevalue'] = "Subtype Comparison " + mtype + " Passed"
				elif results_dict[host]['host_id'] in exempthost :
					results_dict[host]['pfe'] = "notafflicted"
					results_dict[host]['pfevalue'] = "Subtype Comparison " + mtype + " Exempt"
			except Exception as e :
				print("Error trying to match items relating to host ", host, " : " , e )
